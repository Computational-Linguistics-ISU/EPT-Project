...
if __name__ == '__main__':
    ...
    ## Metric evaluation
    prediction_folder = "prediction2"
    metric_files = os.listdir(prediction_folder)
    metric_scores = {}
    if len(metric_files) == 0:
        docs = [doc1] * len(sums) + [doc2] * len(sums)
        sums = sums * 2
        ...
        for scorer in scorers:
            ...
            for scorer_name in scorer_names:
                with open(os.path.join(prediction_folder, "metric_"+scorer_name+".tsv"), "w", encoding="utf-8") as f:
                    ...
                    metric_scores[scorer_name] = np.mean(np.array(metric_scores[scorer_name]).reshape((2, -1)), axis=0)
        ...
        for tsv in metric_files:
            ...
            with open(os.path.join(prediction_folder, tsv)) as f:
                ...
                metric_scores[tsv] = np.mean(np.array(metric_scores[metric_scores[tsv]]).reshape((2, -1)), axis=0)
